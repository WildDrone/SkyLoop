...
if grep -q "FASTRTPS_DEFAULT_PROFILES_FILE.*SkyLoop" "$BASHRC" 2>/dev/null; then
    echo "✓ ROS2 FastDDS config already in ~/.bashrc"
else
    echo ""
    echo "# SkyLoop ROS2 Docker communication setup" >> "$BASHRC"
